...
public class ContextManager {
    ...
    public void enterFile(
        ...
        if (this.innerContext instanceof ModuleContext) {
            ...
            if (GenerationParameters.DEBUG_CONTEXT) {
                System.out.println("[" + module.getPhase() + "]: entered " + innerContext.getClass().getSimpleName());
            }
        } else {
            if (GenerationParameters.DEBUG_CONTEXT) {
                // Throwing only in DEBUG mode, since illegal context orders can happen
                // also when the source code is sintactically wrong, and we don't need to
                // ensure correct context ordering in those cases.
                throw new IllegalContextOrder(this.innerContext, FileContext.class);
            }
        }
    }

    public void enterTopLevelDeclaration(
            BiFunction<SemanticsModule, ? super FileContext, ? extends TopLevelDeclarationContext> provideContext
    ) {
        if (this.innerContext instanceof FileContext) {
            this.outerContexts.push(this.innerContext);
            this.innerContext = provideContext.apply(module, ((FileContext) this.innerContext));
            if (GenerationParameters.DEBUG_CONTEXT) {
                System.out.println("[" + module.getPhase() + "]: entered " + innerContext.getClass().getSimpleName());
            }
        } else {
            if (GenerationParameters.DEBUG_CONTEXT) {
                ...
                // also when the source code is sintactically wrong, and we don't need to
                // ensure correct context ordering in those cases.
                throw new IllegalContextOrder(this.innerContext, TopLevelDeclarationContext.class);
            }
        }
    }
    ...
    public void enterOntologyElementDeclaration() {
        if (this.innerContext instanceof OntologyDeclarationSupportContext) {
            ...
            if (GenerationParameters.DEBUG_CONTEXT) {
                ...
                // also when the source code is sintactically wrong, and we don't need to
                // ensure correct context ordering in those cases.
                throw new IllegalContextOrder(this.innerContext, OntologyElementDeclarationContext.class);
            }
        }
    }
    ...
    public void enterProceduralFeatureContainer(
        ...
        if (this.innerContext instanceof TopLevelDeclarationContext) {
            ...
            if (GenerationParameters.DEBUG_CONTEXT) {
                ...
                // also when the source code is sintactically wrong, and we don't need to
                // ensure correct context ordering in those cases.
                throw new IllegalContextOrder(this.innerContext, ProceduralFeatureContainerContext.class);
            }
        }
    }
    ...
    public void enterProceduralFeature(
            BiFunction<? super SemanticsModule, ? super ProceduralFeatureContainerContext,
                    ? extends ProceduralFeatureContext> supplyContext
    ) {
        if (this.innerContext instanceof ProceduralFeatureContainerContext) {
            this.outerContexts.push(this.innerContext);
            this.innerContext = supplyContext.apply(module, ((ProceduralFeatureContainerContext) this.innerContext));
            if (GenerationParameters.DEBUG_CONTEXT) {
                System.out.println("[" + module.getPhase() + "]: entered " + innerContext.getClass().getSimpleName());
            }
        } else {
            if (GenerationParameters.DEBUG_CONTEXT) {
                ...
                // also when the source code is sintactically wrong, and we don't need to
                // ensure correct context ordering in those cases.
                throw new IllegalContextOrder(this.innerContext, ProceduralFeatureContext.class);
            }
        }
    }
    ...
    public void enterSuperSlotInitializer(
        ...
        if (this.innerContext instanceof OntologyElementDeclarationContext) {
            ...
            if (GenerationParameters.DEBUG_CONTEXT) {
                ...
                // also when the source code is sintactically wrong, and we don't need to
                // ensure correct context ordering in those cases.
                throw new IllegalContextOrder(this.innerContext, SuperSlotInitializerContext.class);
            }
        }
    }
    ...
    public void enterEmulatedFile() {
        if (this.innerContext instanceof ProceduralFeatureContainerContext) {
            ...
            if (GenerationParameters.DEBUG_CONTEXT) {
                ...
                // also when the source code is sintactically wrong, and we don't need to
                // ensure correct context ordering in those cases.
                throw new IllegalContextOrder(this.innerContext, EmulatedFileContext.class);
            }
        }
    }
    ...
    public void exit() {
        if (GenerationParameters.DEBUG_CONTEXT) {
            System.out.println("[" + module.getPhase() + "]: exiting " + innerContext.getClass().getSimpleName());
        }
    ...
    public void pushScope() {
        if (this.innerContext instanceof ScopedContext) {
            ...
            if (GenerationParameters.DEBUG_CONTEXT) {
                throw new IllegalContextOrder(this.innerContext, ProceduralScope.class);
            }
        }
    }
    ...
    public SavedContext save() {
        if (GenerationParameters.DEBUG_CONTEXT) {
            System.out.println("[" + module.getPhase() + "]: Saving context (inner=" + (this.innerContext != null
    ...
    public void restore(SavedContext saved) {
        ...
        if (GenerationParameters.DEBUG_CONTEXT) {
            System.out.println("[" + module.getPhase() + "]: Resumed context (inner=" + (this.innerContext != null
                    ? this.innerContext.getClass().getSimpleName()
                    : "null"
            ) + ")");
        }
    ...
    public ContextManager resuming(SavedContext saved) {
        this.restore(saved);
        return this;
    }
    ...
    public class IllegalContextOrder extends RuntimeException {
        public IllegalContextOrder(Context outer, Class<?> attemptedInner) {
            super("[" + module.getPhase() + "]: A <" + (outer != null ? outer.getClass().getName() : "null")
