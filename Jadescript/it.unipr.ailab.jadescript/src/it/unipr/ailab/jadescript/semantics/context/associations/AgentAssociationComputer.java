...
public interface AgentAssociationComputer extends OntologyAssociationComputer {
    ...
    private Stream<AgentAssociation> computeAgentAssociationsFromSupertype() {
        ...
        if (ms.isPresent() && ms.toNullable() instanceof AgentAssociated) {
            return ((AgentAssociated) ms.toNullable()).computeAllAgentAssociations()
    ...
    AgentAssociationComputer EMPTY_AGENT_ASSOCIATIONS = new AgentAssociationComputer() {
        @Override
        ...
        public Stream<OntologyAssociation> computeUsingOntologyAssociations() {
            return EMPTY_ONTOLOGY_ASSOCIATIONS.computeUsingOntologyAssociations();
        }

        @Override
        public Stream<OntologyAssociation> computeCurrentOntologyAssociations() {
            return EMPTY_ONTOLOGY_ASSOCIATIONS.computeCurrentOntologyAssociations();
        }

        @Override
        public Stream<OntologyAssociation> computeForClauseOntologyAssociations() {
            return EMPTY_ONTOLOGY_ASSOCIATIONS.computeForClauseOntologyAssociations();
        }
    ...
    default void debugDumpAgentAssociations(SourceCodeBuilder scb) {
        ...
        computeAllAgentAssociations().forEach((AgentAssociation a) -> a.debugDump(scb));
        scb.close("}");
    }
}
