...
public interface ScopedContext extends Searcheable {
    ...
    ProceduralScope getCurrentScope();



    @Override
    default <T> Stream<T> search(Function<? super Searcheable, Stream<T>> localSearcher) {
        ...
        if (GenerationParameters.DEBUG_SEARCH) {
            System.out.println("[DEBUG_SEARCH]: Searching in " + this.getClass().getName() +
                    (nextSearcheable.isPresent()
                            ? "(searching first in scopes, then next is :" + nextSearcheable.toNullable().getClass().getName() + ")"
                            : "(searching first in scopes)"
                    ));
        }
        return Streams.concat(
                getCurrentScope().search(localSearcher),//recursively search in all scopes
