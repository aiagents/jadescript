...
public class CallableSymbolSignature extends SymbolSignature {
    ...
    public CallableSymbolSignature(String name, IJadescriptType returnType, List<IJadescriptType> parameterTypes) {
        this.name = name;
    ...
    public boolean equals(Object o) {
        ...
        if (getReturnType() != null ? !getReturnType().typeEquals(that.getReturnType()) : that.getReturnType() != null)
            return false;
    ...
    public String toString() {
        final List<IJadescriptType> parameterTypes = getParameterTypes();
        final String parametersPrint = parameterTypes.stream()
                .map(Object::toString)
                .collect(Collectors.joining(", "));
        if (getReturnType().getID().equals(TypeHelper.VOID_TYPEID)) {
            return "procedure " + getName() + " with " + parametersPrint;
        } else {
            return "function " + getName() + "(" + parametersPrint + ") as " + getReturnType();
        }
    }
    ...
    public int hashCode() {
        ...
        result = 31 * result + (getReturnType() != null ? getReturnType().getID().hashCode() : 0);
        result = 31 * result + (getParameterTypes() != null ? ((Supplier<Integer>) (() -> {
            int paramListHash = 1;
            for (IJadescriptType e : getParameterTypes()) {
                paramListHash = 31 * paramListHash + (e == null ? 0 : e.getID().hashCode());
            }
