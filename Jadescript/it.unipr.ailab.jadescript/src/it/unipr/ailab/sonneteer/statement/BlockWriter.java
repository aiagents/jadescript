...
public class BlockWriter extends StatementWriter {
    ...
    private LocalVarBindingProvider bindingProvider = CompilationHelper.userBlockLocalVars;
    ...
    public BlockWriter addStatement(int index, StatementWriter statementWriter) {
    ...
    public BlockWriter addStatements(List<StatementWriter> statementWriterList) {
    ...
    public void writeSonnet(SourceCodeBuilder s) {
        ...
        s.line("{").indent();
        {
            for (int i = 0; i < elements.size(); i++) {
                StatementWriter st = performBinding(bindingProvider, elements.get(i));
                st.writeSonnet(s);
    ...
    private static StatementWriter performBinding(
        ...
        if(element instanceof StatementWriter){
            return ((StatementWriter) element).bindLocalVarUsages(localVarBindingProvider);
            ...
            return localVarBindingProvider.bindDeclaration(
                    ...
                    ((VariableDeclarationPlaceholder) element).getExpressionWriter().bindVariableUsages(localVarBindingProvider));
            ...
            return localVarBindingProvider.bindWrite(
                    ...
                    ((VariableAssignmentPlaceholder) element).getExpression().bindVariableUsages(localVarBindingProvider)
            );
        }
        throw new RuntimeException("Invalid BlockWriterElement found: "+element.getClass().getName());
    }
        ...
        for (BlockWriterElement element : elements) {
            if (element instanceof StatementWriter) {
                ...
                if (shouldDig.test(statement)) {
                    statement.getSubBlocks(bw -> bw.replaceElements(replace, shouldDig));
                }
            }
        }
    }
